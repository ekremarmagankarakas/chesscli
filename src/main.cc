...
const char* kUsage =
    "Usage: chesscli [--tui] [--no-tui] [--text] [--engine DEPTH]\n"
    ...
    "  --no-tui         Force CLI view instead of TUI\n"
    ...
    "\n"
    "In-game commands (CLI + TUI): move (e.g. e2e4, Nf3, O-O, e7e8q),\n"
...
int main(int argc, char* argv[]) {
  ...
  std::optional<Color> override_engine_side;
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--tui") {
      ...
      override_engine_side = Color::kWhite;
  ...
  if (override_engine_side.has_value()) {
    cfg.play_black = (*override_engine_side == Color::kWhite);
  }
